...
describe(`Survey Import Scenarios`, () => {
  describe(`when the user has sufficient permissions to execute commands`, () => {
    beforeAll(async () => {
        ...
        it(`should create the finalized survey`, async () => {
          await assertCommandScenarioSuccess({
            ...
            assertSuccess: async (acks) => {
              ...
              expect(foundFollowupQuestion.prompt).toBe(
              ...
              const assertOptionHasFlag = ({
                ...
                if (flag.description) {
                  expect(targetFlagForOption.description).toBe(
                    flag.description,
                  );
                }
              };
    ...
    describe(`when the import is invalid`, () => {
      ...
      describe(`when an analyzer is invalid`, () => {
        ...
        describe.skip(`when it has no categories`, () => {
          const invalidAnalyzerDto = buildTestInstance(
            SurveyAnalyzerImportDto,
            {
              categories: [],
            },
          ...
          const importWithEmptyAnalyzer = TestCommandStream.first(
            ImportSurvey,
            {
              name: {
                text: surveyName,
  ...
  describe(`when the user has insufficient permissions to execute commands`, () => {
    describe(`when the user is not signed in`, () => {
      it(`should return not authorized`, async () => {
        await assertCommandScenarioError({
          httpClient: new TestHttpClient('localhost:4200'),
          endpoint: surveyCommandsEndpoint,
          stream: TestCommandStream.first(ImportSurvey),
          assertErrorMessageAsExpected: (message) => {
    ...
    // This would test that ordinary users can't push through survey imports. Only admin can do this at present.
    describe(`when the user is signed in as an ordinary user`, () => {
